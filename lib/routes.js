...
import {
  ...
  mergeAbortChoice, opResponse, opTimeoutMs, stashPull, stashSwitch, unrelatedChoices,
} from './ops.js'
...
/**
 * 只读接口的跨站守卫：**没有 Origin 就放行**（浏览器地址栏直开、curl、监控脚本
 * 都不带这个头），带了 Origin 就必须同源。
 *
 * 与 sameOrigin 的差别是「缺头时的默认值」：POST 是失败关闭（缺头即拒绝），
 * 只读的日志尾读是失败开放（缺头即放行）—— 后者只是不想让别的站点用脚本
 * 把本机的工作痕迹拉走，不是鉴权。HTTP 的跨站简单请求确实可以不带 Origin，
 * 所以这一层挡不住一切；它能挡住的是带来源的脚本化读取，代价为零。
 */
...
function createRoutes(getDefaultDir) {
  ...
  const logRoute = {
    ...
    path: '/git-panel/log',
    handler: async (request, response) => {
      if (!crossOriginAllowed(request)) {
        sendJson(response, 403, { ok: false, message: '拒绝读取：请求来源不可信', state: null })
        return
      }
      try {
