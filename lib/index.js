  ...
  const opRoute = {
    ...
    handler: async (request, response) => {
      try {
        ...
        // 镜像一旦失败就自动回退直连（代理参数保留 —— 那是用户自己的线路，没理由绕开）。
        ...
        const recovery = await recoverPush(op, body, fullArgv, result, dir, timeoutMs, attemptArgs)
        ...
        // 失败原因分流：网络不通要给出「去开加速」的提示，其余交给各自的推送提示。
        ...
        const hint = failure === null
          ? null
          : (networkFailure !== null ? networkHint(accel.mode !== 'direct') : pushHint(recovery.reason))
