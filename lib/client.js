  ...
  factory: (require) => {
    ...
    const S = {
      ...
      help: {
        ...
        borderRadius: '8px', border: '1px solid var(--dsw-alias-border-l1, #eeeeee)',
        background: 'var(--dsw-alias-bg-layer-1, #ffffff)',
      },
      helpTitle: { fontWeight: 600, marginTop: '4px', color: 'var(--dsw-alias-label-primary, #111111)' },
      helpList: { display: 'flex', flexDirection: 'column', gap: '4px' },
      helpDesc: { color: 'var(--dsw-alias-label-secondary, #666666)' },
      helpItem: { display: 'flex', flexDirection: 'column', gap: '2px' },
      helpCmd: {
        textAlign: 'left', fontFamily: 'ui-monospace, Menlo, monospace', fontSize: '11px',
        padding: '5px 8px', borderRadius: '7px', cursor: 'pointer',
        border: '1px solid var(--dsw-alias-border-l2, #dddddd)',
        background: 'var(--dsw-alias-markdown-code-block, rgba(0,0,0,.05))',
        color: 'var(--dsw-alias-label-primary, #111111)',
      },
      /** 帮助窗口：独立于主面板的可移动小窗（拖标题栏移动、右下角手柄拉大小）。
       *  高度：未拉伸过用 72vh，拉伸过用记住的像素高。
       *  滚动：标题栏固定 34px，正文区用 absolute(top:34px; bottom:0) 从父高度
       *  直接推出精确高度——整条链路不用 flex，避免引擎差异导致滚动区高度失效。 */
      helpwin: {
        position: 'fixed', width: '480px', maxWidth: 'calc(100vw - 24px)',
        display: 'block', zIndex: 95, pointerEvents: 'auto',
        background: 'var(--dsw-alias-bg-overlay, #ffffff)',
        color: 'var(--dsw-alias-label-primary, #111111)',
        border: '1px solid var(--dsw-alias-border-l2, #dddddd)',
        borderRadius: '12px', boxShadow: '0 12px 32px rgba(0,0,0,.22)',
      ...
      /** 正文区：absolute 定位 + top/bottom 夹出的确定高度 + 自身滚动。 */
      helpwinBody: {
        position: 'absolute', top: '34px', left: '0', right: '0', bottom: '0',
        padding: '10px 12px 24px', boxSizing: 'border-box',
        display: 'flex', flexDirection: 'column', gap: '6px',
        overflowY: 'auto', overflowX: 'hidden', overscrollBehavior: 'contain',
      },
      helpwinDrag: { color: 'var(--dsw-alias-label-tertiary, #999999)', fontWeight: 400, fontSize: '11px' },
      /** 右下角的拉伸手柄（与标题栏拖拽互补：移动靠标题栏，大小靠手柄）。 */
      helpwinResize: {
        position: 'absolute', right: '0', bottom: '0', width: '18px', height: '18px',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        cursor: 'nwse-resize', color: 'var(--dsw-alias-label-tertiary, #999999)',
        fontSize: '11px', userSelect: 'none', touchAction: 'none',
    ...
    // ── 帮助内容：面向不熟 git 的用户 ─────────────────────────────────────
    // 面板用法（纯文本）+ 分组常用命令（cmd 一行展示，点击即复制）。

    const HELP_HOWTO = [
      '查更新：「获取远程」→ 看分支行「落后 N」，N>0 就是有更新；确认后再点「拉取」合并。',
      '提交：「全部暂存」→ 提交框写说明 → 回车。推送失败后面板会提示下一步点哪里。',
      '看改动：改动清单里点任意条目，直接展开这个文件的 diff；再点一次收起。',
      '切分支：分支行点「管理」→ 点分支名切换；输入新名字可新建并切换。',
      '救场：改动没弄完想切分支？先「全部暂存」或先提交，再切，就不会丢。',
    ]
    ...
    const HELP_POS_KEY = 'dsh-git-panel-help-pos'
    const HELP_SIZE_KEY = 'dsh-git-panel-help-size'
    const HELP_WIN_WIDTH = 480

    /** 读取上次保存的窗口尺寸；没有、损坏或过小时返回 null（用自适应默认）。 */
    function readHelpSize() {
      try {
        const raw = window.localStorage.getItem(HELP_SIZE_KEY)
        if (raw !== null) {
          const parsed = JSON.parse(raw)
          if (parsed !== null && typeof parsed === 'object'
            && typeof parsed.w === 'number' && typeof parsed.h === 'number'
            && parsed.w >= 340 && parsed.h >= 160) {
    ...
    /** 读取上次保存的窗口位置；没有或损坏时返回 null（用默认位置）。 */
    function readHelpPos() {
      try {
        const raw = window.localStorage.getItem(HELP_POS_KEY)
        if (raw !== null) {
          const parsed = JSON.parse(raw)
          if (parsed !== null && typeof parsed === 'object'
    ...
    /** 默认位置：屏幕右上角（避开右下角的主面板）。 */
    function defaultHelpPos() {
      const width = typeof window !== 'undefined' && window.innerWidth > 0 ? window.innerWidth : 1280
      return { x: width - HELP_WIN_WIDTH - 20, y: 16 }
    }
    ...
    function GitPanel(props) {
      ...
      const [helpPos, setHelpPos] = React.useState(() => readHelpPos() ?? defaultHelpPos())
      const [helpSize, setHelpSize] = React.useState(() => readHelpSize())
      const helpBodyRef = React.useRef(null)
      const helpWheelLogged = React.useRef(0)
      ...
      /**
       * 复制命令到剪贴板：优先 Clipboard API（localhost 属安全上下文，可用），
       * 不行退回隐藏 textarea + execCommand；成功后命令显示 ✓ 反馈 1.5 秒。
       */
      const copyText = async (text, key) => {
        ...
        if (ok === true) {
          ...
          setHelpError('复制失败：请手动选中命令文本复制（Ctrl+C / Cmd+C）')
        }
      }
      ...
      // 拖动后把位置存进 localStorage，刷新页面 / 重开插件位置不变。
      React.useEffect(() => {
        try {
          window.localStorage.setItem(HELP_POS_KEY, JSON.stringify(helpPos))
        } catch (error) {
          /* 忽略 */
        }
      }, [helpPos])

      // 拉伸后把尺寸存进 localStorage（helpSize 为 null 表示从未拉伸过）。
      React.useEffect(() => {
        try {
          window.localStorage.setItem(HELP_SIZE_KEY, JSON.stringify(helpSize))
        } catch (error) {
          /* 忽略 */
        }
      }, [helpSize])

      // 拖标题栏移动帮助窗口：用 pointer 事件（鼠标/触摸统一），
      // 拖动中把窗口位置 clamp 在视口内，至少留 90px 可见，避免拖丢。
      const helpDragStart = (event) => {
        if (event.pointerType === 'mouse' && event.button !== 0) return
        event.preventDefault()
        const startX = event.clientX
        const startY = event.clientY
        const originX = helpPos.x
        const originY = helpPos.y
        const onMove = (ev) => {
          const width = typeof window !== 'undefined' && window.innerWidth > 0 ? window.innerWidth : 1280
          ...
          x = Math.min(Math.max(x, 90 - HELP_WIN_WIDTH), width - 90)
          y = Math.min(Math.max(y, 0), Math.max(0, height - 40))
          setHelpPos({ x: x, y: y })
        }
        const onUp = () => {
      ...
      // 拖右下角手柄拉伸窗口：宽度 340px 起、高度 160px 起，
      // 且不超出视口剩余空间（窗口右/下边缘不越界）。
      const helpResizeStart = (event) => {
        if (event.pointerType === 'mouse' && event.button !== 0) return
        event.preventDefault()
        event.stopPropagation()
        const startX = event.clientX
        const startY = event.clientY
        const originW = helpSize !== null ? helpSize.w : HELP_WIN_WIDTH
        const originH = helpSize !== null
          ? helpSize.h
        ...
        const onMove = (ev) => {
          ...
          const x = helpPos.x
          const y = helpPos.y
          let w = originW + ev.clientX - startX
          let h = originH + ev.clientY - startY
          w = Math.min(Math.max(w, 340), Math.max(340, width - x - 8))
          h = Math.min(Math.max(h, 160), Math.max(160, height - y - 8))
          setHelpSize({ w: Math.round(w), h: Math.round(h) })
        }
        const onUp = () => {
          window.removeEventListener('pointermove', onMove)
          window.removeEventListener('pointerup', onUp)
      ...
      /**
       * 诊断上报：把帮助窗口的真实几何与滚动状态写进宿主日志
       * （~/.dsh/git-panel-diag.log）。滚动再出问题时不靠猜：直接看正文区
       * 到底有没有溢出（scrollHeight vs clientHeight）、overflowY 是否生效、
       * 光标下命中的是哪个元素、祖先里有没有 overflow/transform 破坏 fixed 与滚动。
       */
      const logHelpGeometry = (stage, event) => {
        try {
          const body = helpBodyRef.current
          const win = body !== null && body !== undefined ? body.parentElement : null
          const hasEvent = event !== undefined && event !== null
          const info = {
            stage: stage,
            vw: window.innerWidth,
            vh: window.innerHeight,
            docH: document.documentElement !== null ? document.documentElement.clientHeight : null,
            // 窗口在视口里的真实矩形（fixed 元素用 getBoundingClientRect 最可靠）
            rect: win === null
              ? null
              : (() => {
            ...
            // 存进 localStorage 的位置/尺寸——用来排除"存坏了导致窗口超出屏幕"
            storedPos: readHelpPos(),
            storedSize: readHelpSize(),
            win: win === null
              ? null
              : { w: win.offsetWidth, h: win.offsetHeight, ch: win.clientHeight, left: win.offsetLeft, top: win.offsetTop },
            body: body === null || body === undefined
              ...
              : {
                  ...
                  oy: window.getComputedStyle(body).overflowY,
                },
            target: hasEvent ? String(event.target && event.target.tagName) : null,
            at: hasEvent
              ? (() => {
                  const el = document.elementFromPoint(event.clientX, event.clientY)
                  return el === null || el === undefined
                    ? null
                    : String(el.tagName) + '|' + String(el.className).slice(0, 40)
                })()
              : null,
            ancestors: (() => {
              const out = []
              ...
              for (let depth = 0; node !== null && node !== undefined && depth < 5; depth += 1) {
                ...
                out.push(
                  String(node.tagName) + '|' + String(node.className).slice(0, 24) +
                  '|pos=' + style.position + '|ov=' + style.overflow + '|tf=' + style.transform,
                )
          ...
          diag('help:' + stage, JSON.stringify(info))
        } catch (error) {
          /* 诊断失败不影响功能 */
        }
      }

      // 打开或改变尺寸后上报一次：确认滚动区高度是否真的生效。
      React.useEffect(() => {
        if (showHelp !== true) return undefined
        const timer = window.setTimeout(() => logHelpGeometry('open'), 80)
        return () => window.clearTimeout(timer)
      }, [showHelp, helpSize, helpPos])

      // 滚轮时上报（最多 8 次，含 150ms 后的滚动位置）：判断是事件没到还是没得滚。
      const onHelpWheel = (event) => {
        if (helpWheelLogged.current >= 8) return
        helpWheelLogged.current += 1
        logHelpGeometry('wheel', event)
        window.setTimeout(() => logHelpGeometry('wheel-after'), 150)
      }

      // 帮助窗口本体：showHelp 时构建为 fixed 小窗，位置跟随 helpPos；
      // 内容与之前内嵌版相同（操作方式 + 可复制命令），但横向更宽、可拖可关。
      let helpWindow = null
      if (showHelp === true) {
        const helpChildren = [
          h('div', {
            style: S.helpwinHead, key: 'head',
            title: '按住标题栏拖动窗口',
            onPointerDown: helpDragStart,
          },
            h('span', null, '📖 Git 帮助'),
            h('span', { style: S.helpwinDrag }, '拖标题栏移动 · 拖右下角拉大小'),
            h('span', { style: S.spacer }),
            h('button', { style: S.mini, title: '关闭帮助窗口', onClick: () => setShowHelp(false) }, '×'),
          ),
          h('div', { style: S.helpwinBody, key: 'body', ref: helpBodyRef },
            h('div', { style: S.helpTitle, key: 'ht' }, '面板操作方式'),
            h('div', { style: S.helpList, key: 'howto' },
              HELP_HOWTO.map((text, index) => h('div', { style: S.helpDesc, key: 'h' + index }, '· ' + text)),
            ),
          ),
        ]
        for (const section of HELP_SECTIONS) {
          const keyPrefix = 'cmd:' + section.title
          helpChildren.push(h('div', { style: S.helpTitle, key: section.title }, section.title))
          helpChildren.push(h('div', { style: S.helpList, key: 'clist:' + section.title },
            section.cmds.map((item, index) => {
              const key = keyPrefix + index
              const copied = copiedKey === key
              return h('div', { style: S.helpItem, key: 'i' + index },
                h('div', { style: S.helpDesc }, item.desc),
                h('button', {
                  style: copied
                    ? Object.assign({}, S.helpCmd, { color: 'var(--dsw-alias-state-success-primary, #16a34a)', borderColor: 'var(--dsw-alias-state-success-primary, #16a34a)' })
                    : S.helpCmd,
                  title: '点击复制到剪贴板',
                  onClick: () => copyText(item.cmd, key),
                }, copied ? '✓ ' + item.cmd : item.cmd),
              )
            }),
          ))
        }
        if (helpError.length > 0) {
          helpChildren.push(h('div', {
            style: Object.assign({}, S.helpDesc, { color: 'var(--dsw-alias-state-danger-primary, #dc2626)' }),
            key: 'err',
          }, '⚠ ' + helpError))
        }
        // 右下角拉伸手柄（与标题栏拖拽互补：移动靠标题栏，大小靠手柄）。
        helpChildren.push(h('div', {
          style: S.helpwinResize,
          title: '拖动调整窗口大小',
          onPointerDown: helpResizeStart,
        }, '↘'))
        helpWindow = h('div', {
          style: Object.assign({}, S.helpwin,
            {
              left: helpPos.x + 'px',
              top: helpPos.y + 'px',
            },
            // 未拉伸过：高度 72vh（浏览器随视口自适应）；拉伸过：用记住的像素高。
            helpSize !== null ? { width: helpSize.w + 'px', height: helpSize.h + 'px' } : { height: '72vh' }),
          key: 'helpwin',
          onWheel: onHelpWheel,
        }, helpChildren)
      }
      ...
      const head = h('div', { style: S.head, key: 'head' },
        ...
        h('button', {
          style: S.mini,
          title: showHelp === true ? '收起帮助窗口' : '帮助：打开可移动窗口，查看操作方式与常用命令（点命令即可复制）',
          onClick: () => setShowHelp(showHelp !== true),
        }, '?'),
        h('button', {
          style: S.mini,
          title: '最小化',
      ...
      return h(React.Fragment, null,
        h('div', { style: S.panel }, head, h('div', { style: S.body }, children)),
        helpWindow,
      )
    }
