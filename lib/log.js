...
/**
 * 日志文件路径：显式指定优先，否则落在**启动 dsh 时所在的工作区目录**
 * （`process.cwd()`，也就是 `git-panel.log` 就在你眼前，tail 它不用先 cd 到
 * `$DSH_HOME`）。文件名固定，已在 `.gitignore` 里排除，不会被提交。
 */
function logFilePath() {
  return (logConfig.file !== null && logConfig.file !== undefined && String(logConfig.file).trim().length > 0)
    ? String(logConfig.file)
    : join(process.cwd(), 'git-panel.log')
}
