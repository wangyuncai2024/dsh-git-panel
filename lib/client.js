  ...
  factory: (require) => {
    ...
    const S = {
      ...
      netBox: {
        ...
        borderRadius: '8px', border: '1px solid var(--dsw-alias-border-l2, #dddddd)',
        background: 'var(--dsw-alias-bg-layer-1, #ffffff)',
      },
      ...
      select: {
        flex: '1 1 auto', minWidth: 0, fontSize: '12px', padding: '4px 6px', borderRadius: '7px',
        border: '1px solid var(--dsw-alias-border-l2, #dddddd)',
        background: 'var(--dsw-alias-bg-layer-1, #ffffff)',
        color: 'var(--dsw-alias-label-primary, #111111)',
      },
    ...
    function GitPanel(props) {
      ...
      if (showNet === true) {
        ...
        if (Array.isArray(netProbe)) {
          netChildren.push(h('div', { style: S.list, key: 'results' },
            netProbe.map((item, index) => h('div', { style: S.item, key: 'p' + index },
      ...
      if (isRepo === true && showBranches === true) {
        ...
        const remoteItems = remoteBranches !== null && Array.isArray(remoteBranches.items)
          ...
          : []
        children.push(h('div', { style: S.note, key: 'remotetitle' },
          remoteItems.length === 0
            ? '远端分支：点一次「获取远程」就能在这里看到（只下载，不改你的代码）'
            : '远端分支（「拿成新分支」把远端那份取到本地，当前分支一点不动）'))
        for (let index = 0; index < remoteItems.length; index += 1) {
          const item = remoteItems[index]
          children.push(h('div', { style: S.item, key: 'r' + index },
            h('span', {
              style: S.name,
