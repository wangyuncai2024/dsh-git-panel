  ...
  factory: (require) => {
    ...
    function useGitPanel(props) {
      ...
      const openPicker = () => {
        ...
        patch({
          pickerOpen: true, pickLevel: null, pickSelected: null, pickBusy: false,
      ...
      const closePicker = () => {
        ...
        patch({
          pickerOpen: false, pickLevel: null, pickSelected: null, pickBusy: false,
          pickError: '', pickShowHidden: false, pickDraft: null, pickFolder: null,
          pickCreating: false, pickCreateError: '',
        })
      }
      ...
      /** 提交路径：有浏览能力就列那个目录；没有就直接切过去（一步到位）。 */
          ...
          h('div', { style: { display: 'flex', alignItems: 'center', gap: '6px', padding: '10px 14px 0' } },
            editing
              ? h('input', {
                  ...
                  className: 'dgp-input',
                  value: s.pickDraft,
                  placeholder: '绝对路径（回车进入），如 /home/me/project',
                  autoFocus: true,
    ...
    function apply(ctx) {
       ...
       * 同一个选择器（uiWorkspace 的 listDirectory / createDirectory，宿主那个
       * 对话框内部走的也是这两个方法）。
       *
