...
function spawnFailureText(raw, cwd) {
  ...
  if (!/ENOENT|not found/i.test(detail)) return detail
...
const NOT_REPO_PATTERN = /not a git repository|not a git repo|not a working (tree|copy)/i
...
export {
  ...
  parseRemoteBranchOutput, isSafeRemoteRef, parseCompareOutput,
