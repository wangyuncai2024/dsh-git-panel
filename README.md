...
| 自动刷新 | 窗口重新获得焦点 / 标签页重新可见时**静默**读一次状态（不点亮「同步中…」、失败不刷结果栏）；**有未提交改动每 20 秒、工作区干净每 60 秒**各轮询一次 —— 编辑器 / 终端 / AI 工具在面板之外产生的改动（包括切分支、别人推来的提交）都会自己出现在面板上；展开着的**分支管理器**也会随状态重列，外部删掉的分支不会留在列表里（见 [0.14.4](#0144面板之外的改动会自己出现无契约变更)） |
...
| 切换 / 刷新 | 「切换」**弹出目录选择小窗口**（与 DSH「添加工作区」同一个选择器：浏览目录、点面包屑回退、双击进入子目录、显示隐藏目录、新建文件夹，也可直接输入绝对路径）；宿主这次只组合了**系统对话框**时，小窗口自动换成「打开系统目录选择器…」那一版（见 [0.14.1](#0141宿主只组合了系统对话框时小窗口改走系统对话框无契约变更)）；「刷新」重新读取状态 |
...
| **本地分支名和它跟踪的远端分支名不一致** | 给**三条可点的路**（推到上游那个分支 / 在远端另建同名分支 / 把本地名改成与上游一致），见 [0.15](#015本地分支名和远端对不上时面板直接把路铺好) |
...
- **同源校验**：执行类 POST 请求校验 `Origin`，拒绝跨站提交；**不可判定的来源不算跨站**
  （缺 `Origin`、`Origin: null`、`file://` / 自定义协议、本机别名与回环端口，0.14.3 起）。
...
- **统一操作日志**：面板操作、AI 工具调用、网络配置变更、客户端注册过程、内部错误
  全部写进**本插件仓库根目录**下的 `git-panel.log`（JSONL，级别 `off/error/warn/info/debug` 可配），
...
- **同源校验之外再加一次性令牌**（0.12）：`sameOrigin` 比的是客户端自己写的两个头，
  ...
  **0.14.3 起同源校验放宽**：只拒绝明确的跨站 http(s) 来源，令牌成为唯一的硬门槛
  （原委见 [0.14.3](#0143桌面版electron-外壳的-post-不再被同源守卫误伤)）。
...
- **日志落在工作区**（0.12）：默认写到 `process.cwd()/git-panel.log`（启动 dsh 时所在
  ...
  **0.14.2 起默认位置改为插件仓库根目录**：启动目录可能正是宿主的 checkout 或别人的
  仓库，往里丢日志属于「往别人的仓库里写文件」，见
  [0.14.2](#0142日志固定写在插件仓库根目录无契约变更)。
...
| 日志默认路径 | `$DSH_HOME/git-panel.log` | **启动目录**下的 `git-panel.log`（0.14.2 起改为**插件仓库根目录**） | 日志就在工作区里，`tail -f` 不用先 cd；已在 `.gitignore` 排除 |
...
## 0.15：本地分支名和远端对不上时，面板直接把路铺好

**问题（真实发生）**：本地分支叫 `origin-main`、它跟踪的是 `origin/main`，点「推送」只得到一句

```
fatal: The upstream branch of your current branch does not match the name of your current branch.
```

—— 没有中文、没有下一步（git 自己给的两条命令也都是英文的），用户只能去猜。同一次操作里还
撞上第二堵墙：整理仓库时点「删除」，删一个未合并的分支，只回 `error: the branch 'main' is not
fully merged` —— 同样没人告诉他「这是保护」以及「确实不要了该怎么办」。而这一串混乱的源头，
是面板允许把本地分支建成 `origin/main` 这种**和远端跟踪引用同名**的名字。

所以三处一起修：**提前说**（状态条）、**失败后把路铺好**（可点的选项）、**从源头拦住**（名字打错时就地拒）。

| 位置 | 之前 | 现在 |
| --- | --- | --- |
| `classifyPushFailure` | 这条报错落到 `none`，面板只能甩英文原文 | 新增一类 `upstream-name-mismatch`（排在 `rejected` 之前，因为它没有 `failed to push some refs` 字样），配中文 hint |
| push 失败后的选项 | 无 | **三条路**（`mismatchPushChoices`）：① 推到上游跟踪的 `origin/main`（`git push origin HEAD:main`，本地名一点不动）② 在远端另建同名分支 `origin-main`（`git push --set-upstream origin HEAD`，上游改成它）③ 把本地分支改名成 `main`（改完点一次「推送」即可，改名有二次确认） |
| 状态条 | 只有失败之后才可能知道 | 本地名与上游名不一致时提前挂一个 **名称不一致** 标签（悬停说明「默认配置下裸 push 会被 git 拒绝」） |
| 删分支被拒 | 只回 `not fully merged` 英文 | 分类成 `unmerged` + 中文解释（说清这是 git 的防误删保护）+ 一条**强制删除（`branch -D`）** 选项；确认文案写明「未合并的提交会失去分支引用 / 可用 `git reflog` 找回（有时间限制）/ 不可逆」 |
| 改分支名撞名 | 只回 `A branch named 'x' already exists` | 分类成 `exists` + 提示指向「推到上游」那条**不用改名**的路 |
| 新建分支 | `origin/main` 这类名字照建不误 —— 本地分支与 `refs/remotes/origin/main` 从此同名（`git branch -a` 开始报 ambiguous），它的第一次推送还会在远端造出一个同样叫 `origin/main` 的分支 | 三层拦住：输入框旁就地告警 + 「新建」按钮锁住；回车那条路在动作里再挡一次（它不经过按钮的 disabled）；宿主与 AI 工具共用的 `assertSafeNewBranch` 兜底（读一次 `git remote -v`，`branchNameRemoteConflict` 同一判定）。`feat/x` 这类正常名字不受影响 |
| AI 工具（`git_*`） | 失败只回 git 英文原文（面板有中文提示，两半不一致） | 失败时追加注册表里的同一份 hint（`⇢ …`），模型与用户都能看到下一步 |
| 两个远程指向同一地址（本次现场：`main` + `origin` 同 URL） | 面板只显示按名称排序的第一个，重复的那个悄悄待着 —— 可它会让 `git log main` / `git branch -D main` 报 `refname 'main' is ambiguous` | 宿主在状态里给出分组（`git.js` 的 `duplicateRemotes`，保留 `origin`），「远程」区多一行警告 + 每个多余远程一个「删掉 X」按钮（二次确认写明「只动本地配置与 `refs/remotes/X/*`，服务器与提交不受影响」） |
| 上游信息的取法 | — | 「本地名 ≠ 上游名」时先问 `@{upstream}`；它在「远端跟踪引用还没 fetch 到本地」时会解析失败 —— 而现场常常正是这样，于是再退一步读 `branch.<名字>.remote/merge` 配置 |
| 测试 | — | 新增 8 条纯函数用例、2 条客户端用例、2 条**真 git** 的端到端用例（造出 `origin-main → origin/main` 现场跑 push 并断言三条路；造出未合并分支跑 `-d` 与 `-D`）；顺手用 `GIT_CONFIG_*` 治好 Windows 上 `core.autocrlf=true` 造成的 4 条假失败 |

那三条路都是**显式 refspec**（`HEAD:main` / `--set-upstream <远程> HEAD`），一律以参数注入，
**不改用户的 `push.default` 或任何 git 配置** —— 与插件「只作用于单条命令」的既有原则一致。

## 0.14.4：面板之外的改动会自己出现（无契约变更）

**问题（真实发生）**：用 AI 工具（本插件的 `git_*`）或终端把仓库整理好之后 —— 分支改名、
删掉多余分支、推送 —— 面板上的分支名与领先 / 落后数**原地不动**：「为啥面板还没有改变」。
这里其实有**两个**缺口：面板不肯重新读状态（轮询条件太窄），以及分支列表只在展开那一刻
拉一次（外部删掉的分支会一直挂在列表里）。两个都补掉了。

原因不是数据没更新。直接问宿主，它给的已经是新的：

```bash
curl 'http://127.0.0.1:19387/git-panel/state?dir=C:/path/to/repo'
# {"ok":true,"isRepo":true,"branch":"main","upstream":"origin/main","ahead":0,"behind":0,"changes":[],...}
```

面板显示的是**浏览器那一刻的 state**，而客户端只在两种情况下重新去读：

| 触发 | 条件 |
| --- | --- |
| 窗口重获焦点 / 标签页重新可见 | 一直坐在同一个标签页里打字时，这两个事件都不会发生 |
| 每 20 秒轮询 | 只有 `changesTotal > 0` 才注册定时器 |

整理仓库恰好是在**干净**的工作区上做的 —— 后一个条件不成立，前一个不触发，于是面板一直
停在旧快照上，只能靠手动点「刷新」或碰巧切一次标签页。

| 位置 | 之前 | 现在 |
| --- | --- | --- |
| `lib/client.js` 的轮询 effect | 只有 `changesTotal > 0` 才注册定时器（20 秒） | 只要是仓库就注册：有改动 20 秒（`POLL_DIRTY_MS`）、**干净 60 秒**（`POLL_CLEAN_MS`） |
| effect 依赖数组 | `[workdir, changesTotal]` | `[workdir, isRepo, changesTotal]`，仍然只放**原始值** —— `state.snapshot` 每次轮询都是新对象，放进依赖会让定时器每轮被清掉重建，那样它永远等不到下一次 |
| 分支管理器的列表 | 只在展开时、以及面板自己做完分支操作后刷新 | 多一条「仓库状态一变就重列」的 effect（与 stash 备份区那条同构，只在展开状态下跑）：外部的切分支 / 新建 / 删除也会落到列表上，刚删掉的分支不会继续挂在那里 |
| 测试替身 | 状态响应固定返回**同一个对象**，`state.snapshot` 的对象身份永远不变 —— 依赖它的 effect 在测试里不可能重跑，这条行为根本测不出来 | `options.stateResponse` 可以传函数：每次请求返回**新对象**，与真实 `fetch(...).json()` 一致（传静态对象的老用例不受影响） |
| 测试 | 替身 `window` 根本没有 `setInterval` | 替身补上 `setInterval` / `clearInterval`；新增两个用例：轮询档位（有改动 20 秒 / 干净 60 秒 / 不是仓库不注册）、分支列表随状态重列（外部删掉的分支必须消失） |

代价：一次状态读 = 4 条本地 git（`rev-parse` + `status` + `log` + `remote`），干净时每
分钟一次。换来的是「面板之外的任何改动最多 60 秒自己出现」，不必再记得点刷新。

**客户端代码改完要刷新一次页面**才生效（客户端 bundle 按文件内容重算版本号，见
「改代码后怎么生效」一节）；宿主半边这次没有改动，不需要重启 dsh。

## 0.14.3：桌面版（Electron 外壳）的 POST 不再被同源守卫误伤

**问题（真实发生）**：在**桌面版**里点「获取远程」，面板只回一句
`拒绝执行：请求来源不可信` —— 别的什么线索都没有。原因在宿主半边那条约 10 行的
`sameOrigin`：它要求 `Origin` 与 `Host` **逐字同源**，而桌面版（Electron 外壳）里的页面
不一定从 `http://<同一个 Host>` 打开 —— 外壳可能不带 `Origin`、带 `Origin: null`
（file:// 或沙箱 iframe）、带 `file://` / 自定义协议，或者自己起一个回环端口做转发
（转发后 `Host` 是宿主的端口、`Origin` 是外壳的端口）。这些形态全被判成跨站。

更难受的是**失败只发生在 POST 上**：`GET /git-panel/state` 不校验来源，所以面板照常
显示仓库状态、分支、改动 —— 看起来一切正常，只有每一个动作（获取远程 / 拉取 / 推送 /
提交 / 保存网络设置）回同一句没头没尾的拒绝。

| 位置 | 之前 | 现在 |
| --- | --- | --- |
| `sameOrigin` / `crossOriginAllowed` | `Origin` 与 `Host` 必须逐字相同，缺头即拒绝 | 只拒绝**明确的跨站 http(s) 来源**；缺 `Origin` / `null` / 解析不了 / 非 http(s)（`file://`、自定义协议）→ 不可判定，放行 |
| 主机比对 | 字符串全等 | 同名主机比端口；`localhost` / `127.0.0.1` / `[::1]` 视为同一台机器（端口可不同，桌面外壳的转发就是这种） |
| 403 响应体 | 只有「请求来源不可信」 | 带上原因：`拒绝执行：请求来源不可信（来源 http://localhost:51999 与本站 127.0.0.1:19387 不同源）` |
| 日志 | 被拒的请求不留痕 | 多一条 `event:"guard"`（`origin` / `host` / `userAgent` / `reason`），误判时一眼能看出是哪种部署形态 |

安全门槛没有变化：**会话令牌才是硬门槛**（跨站页面读不到响应体，拿不到令牌），
`Origin` 一直只是纵深防御 —— 它不该比宿主自己的信任层更严。上面每个形态都补了用例
（`test/network.test.mjs`：桌面形态放行 + 跨站仍 403 + 缺 `Origin` 也仍然要令牌）。

## 0.14.2：日志固定写在插件仓库根目录（无契约变更）

**问题（真实发生）**：在宿主自己的 checkout 里启动 dsh（`dsh web` 的 cwd 就是那个仓库），
再点面板的「刷新」，仓库根目录里就多出一个 `git-panel.log`。原因是默认日志路径取的是
`process.cwd()` —— 那是**启动 dsh 时所在的目录**，与插件无关，可能是宿主的 checkout、
别人的项目、任何被查看的仓库。往那里写日志等于「往别人的仓库里丢文件」。

| 位置 | 之前 | 现在 |
| --- | --- | --- |
| 日志默认路径 | `process.cwd()/git-panel.log`（启动 dsh 的目录） | **本插件仓库根目录**下的 `git-panel.log`（`lib/log.js` 的上一级，由 `import.meta.url` 推导） |
| 依赖关系 | 跟「谁启动的、在哪个会话点的刷新、当前选中哪个仓库」都有关 | 与这三者**全都无关**：路径由插件自身位置决定，装到哪就写到哪 |
| `logFile` 行配置 | 可覆盖 | 不变（仍可覆盖到任意路径，例如集中收集） |

这样面板切换 / 刷新查看的仓库永远不会成为落盘位置；插件源码在哪个仓库，日志就在哪个
仓库。已经误生成在别处的 `git-panel.log` 可以直接删掉（本插件不会再去读它）。

## 0.14.1：宿主只组合了「系统对话框」时，小窗口改走系统对话框（无契约变更）

这一版**只动客户端半边**（`lib/client.js`）：没有新增 / 改名的宿主接口，宿主侧一行没改。

**背景（真实报错）**：以 `127.0.0.1` 启动的本机 DSH，`directory-picker-auto` 组合出来的
目录选择器是 **native（系统对话框）**，而不是 browse（网页里列目录）—— 见
`resolveDirectoryPickerBackend`：loopback 绑定 + 本机图形会话 → native。这两种交互是同一个
remote 命名空间下的**两个能力**：`list` / `createDirectory` 要 browse，`pick` 要 native，
一次启动只组合其中一种，另一种会被明确拒绝：

```
directoryPicker.list needs the browse capability; the composed picker serves "native"
```

面板 0.14 只认 browse 那条路，于是小窗口一打开就把上面这句英文甩给用户 —— 明明宿主的
系统对话框是可用的。

| 位置 | 之前 | 现在 |
| --- | --- | --- |
| 打开小窗口 | 直接把 `directory-picker/unavailable` 的英文消息显示在窗口里 | 认出这是「宿主只组合了系统对话框」，不再报错，换成系统对话框那一版界面 |
| 小窗口内容（native 时） | 列目录的空白区 + 用不上的「新建文件夹 / 显示隐藏文件 / 选择此目录」 | 一句说明 + **「打开系统目录选择器…」**（调 `uiWorkspace.pickDirectory()`）；浏览相关的按钮隐藏 |
| 手输绝对路径 | 回车先去列目录（必然失败） | native 下回车**一步切过去**（与「服务缺失」时的退化路径一致） |
| 系统对话框取消 | — | 小窗口留着，不静默关窗、不换目录 |
| 普通读取失败（如某一层没权限） | 显示宿主的业务消息 | 不变 —— 只有**能力缺席**才切模式，某一层读不了不该把用户踢出网页浏览器 |

判据不是「调用失败」，而是失败码：`rpcError.code === 'directory-picker/unavailable'`
且 `details.capability === 'native'`。每次打开小窗口重新探测一次（只多一次注定失败的
调用），宿主下次以别的绑定方式启动、组合出 browse 时，面板自动回到网页浏览器那一版。

## 0.14：「切换」目录改为弹出目录选择小窗口（无契约变更）
...
| 位置 | 之前 | 现在 |
| --- | --- | --- |
| 仓库卡的「切换」 | 就地展开一个**文本输入框**，要求用户自己敲绝对路径 | **弹出目录选择小窗口** —— 与 DSH「添加工作区」用的是**同一个目录选择器** |
| 小窗口里能做什么 | — | 浏览宿主目录、面包屑回退、双击进子目录、显示隐藏目录、**新建文件夹**、直接输入绝对路径（回车进入） |
| 目录从哪来 | 用户手敲；敲错只有「读不到」 | 宿主的 `uiWorkspace` 服务（`listDirectory` / `createDirectory`）—— **「添加工作区」那个对话框内部走的也是这两个方法**，同一套权限、同一套错误消息 |
| 服务缺失时 | — | 退化成「只能手输绝对路径」，并在小窗口里说明原因；面板本体照常工作（0.14.1 起还处理「服务在、但只组合了系统对话框」那一半，见上一节） |
| 服务怎么拿 | — | 写进 `inject`（`inject = ['slots', 'uiWorkspace']`）后由 cordis 注入 `ctx.uiWorkspace`。**这一点踩过坑**：最初照着 `ctx.remote.directoryPicker` 取、又为了「服务可选」刻意不声明 `inject` —— 而客户端服务是 cordis 懒注入的，没声明的服务在插件上下文里就是 `undefined`，于是小窗口永远显示「宿主没有提供目录浏览服务」。正确做法是声明依赖拿服务，**缺失时在取值处退化**（`uiWorkspace` 缺席也不抛），而不是不声明 |
| 迟到的响应 | — | 与状态请求同一套归属校验：窗口每开/关一次换代、同一次打开内多次列出用序号、在飞的列出用 `AbortController` 掐断 |

为什么不是直接复用宿主那个对话框组件：`DirectoryBrowser` 是 `ui-directory-picker-browse`
包的**内部组件**，第三方插件 `import` 不到（bundle 的 `require` 只拿得到平台种子模块）。
所以面板里实现了一个同交互的紧凑版，但**数据通道与交互语义完全相同** —— 弹的是同一个
宿主目录选择器，选中的路径照旧交给面板自己的 `/git-panel/state?dir=…`。

> 小窗口只回答「面板接下来看哪个目录」：它**不注册 workspace、不切换会话** ——
> 那是 DSH「添加工作区」自己的事（要建 workspace 并开新会话）。两者共用选择器，
> 后续动作各自负责。
...
- id: git-panel
  config:
    ...
    logFile: ''                    # 日志文件路径；留空 = 插件仓库根目录下的 git-panel.log
```
...
`Origin`（只拒绝**明确的跨站 http(s) 来源**，0.14.3 起）与一枚**一次性会话令牌**（0.12 起）。

**硬门槛是令牌**：`GET /git-panel/state` 与 `GET /git-panel/net` 在响应体里各发一枚随机
令牌（保留最近 64 枚，多标签页不会互相踢掉），`POST /op` 与 `POST /net` 必须带上其中一枚
才执行。浏览器发起的跨站请求虽然能发出 GET，却**读不到响应体**，因此拿不到令牌；
带上假令牌一律 403。注意两点：令牌只在浏览器同源页面里流转、不落盘；**服务端从未发放过
令牌时不校验**，这是为了不破坏 `curl` 之类的本地自动化（也正因如此，它不是「鉴权」，
只是加固）。

**`Origin` 只是纵深防御**：只拒绝「明确来自另一个网站」的 http(s) 来源。缺 `Origin`、
`Origin: null`、`file://` / 自定义协议，以及与 `Host` 同为本机别名（`localhost` /
`127.0.0.1` / `[::1]`，端口可以不同）的来源一律放行 —— 桌面版（Electron 外壳）的页面
不一定从与 `Host` 逐字相同的来源打开，早先「逐字同源」的写法会把它的每个 POST 都拒掉
（原委见 [0.14.3](#0143桌面版electron-外壳的-post-不再被同源守卫误伤)）。即便如此：
...
- 放宽来源之后，**本机回环上的其它页面**（例如某个本地 dev server）也不再被来源检查
  挡住 —— 挡住它的仍是令牌，而令牌读不到。真正的风险窗口只剩下「服务端还没发放过任何
  令牌、面板一次都没打开过」的那段时间。
...
`GET /git-panel/log` 是**只读**的日志尾读接口，**明确来自另一个网站的 http(s) 来源一律
拒绝**（地址栏直开、`curl`、桌面外壳不带可比的 `Origin`，照常可用）。日志本身已经是保守
视角：命令参数打码、凭据不落盘；但它包含操作目录、仓库名与提交信息等**本机工作痕迹**，
对局域网开放 web 端口时这些内容同样可见（伪造 `Origin` 或干脆不带 `Origin` 的请求仍能
读到），请按下面的结论一并考虑。
...
插件在本机上维护一份统一的操作日志，**默认固定写在插件仓库根目录**（`lib/log.js` 的
上一级，由 `import.meta.url` 推导）—— 不跟 `process.cwd()` 走，所以不会写进宿主的
checkout 或面板当前查看的任何一个仓库：

```
<插件仓库根目录>/git-panel.log     # JSONL：一行一条 { at, level, event, … }
<插件仓库根目录>/git-panel.log.1   # 超过轮转上限后的旧文件（只保留这一份备份）
```
...
```bash
npm test        # node --test：213 个用例，秒级完成（假 git / 真 git 的 POSIX 专属用例会自动跳过）
...
| 运行中的宿主 HTTP 冒烟 | 对**已经在跑的** dsh 打真实请求（`/git-panel/state`、`/op`、`/net`、`/log`、`/help`，含 400/403/405 与危险参数被挡在 git 之外），并在插件仓库根目录的 `git-panel.log` 里核对留痕 | 「磁盘上是新代码、进程里跑的是旧代码」这件事本身；只有真实进程才有的响应形状（0.10.1 的两个文案缺陷就是这样发现的：敲错目录被报成「未检测到 git」、非仓库显示 git 英文原文） |
