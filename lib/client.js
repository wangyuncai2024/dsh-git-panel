...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const PANEL_CSS = [
      ...
      // 操作成功时结果栏闪一下：结果栏在面板最底部，点「全部暂存」之类的按钮后
      // 用户不一定会往下看 —— 这一下短暂的描边告诉他「刚才那一下有回音」。
    ...
    function useGitPanel(props) {
      ...
      React.useEffect(() => {
        if (typeof state.width === 'number') writeStorage(WIDTH_KEY, state.width)
      }, [state.width])
      ...
      /**
       * 有未提交改动时的轻量轮询（20s）：改动往往是在面板之外产生的（编辑器保存、
       * 终端里的命令），没有这条时胶囊上的红点与改动数会一直停在旧值。
       * 只在标签页可见时跑，隐藏时靠 visibilitychange 那一次补上。
       */
      React.useEffect(() => {
        const total = state.snapshot !== null && typeof state.snapshot.changesTotal === 'number'
          ? state.snapshot.changesTotal
          : 0
        if (total <= 0 || state.snapshot === null || state.snapshot.isRepo !== true) return undefined
      ...
      }, [state.workdir, state.snapshot !== null && state.snapshot.isRepo === true ? state.snapshot.changesTotal : 0])
      ...
      return {
        ...
        actions: {
          ...
          push, saveRemote, saveRemoteAndPush, copyRemote, doCommit, doCommitAndPush,
        },
      }
    }
    ...
    function changeRow(item, open, locked, onToggle, onStage, onUnstage, onRestore) {
      ...
      return h('div', {
        ...
        h('span', { style: codeStyle(staged) }, code),
    ...
    function OutputPanel(props) {
      ...
      return renderLines(props.output, S.out, outputLineClass, 'out', props.outRef)
    }
    ...
    /** 提交表单（回车即提交；可选补充到上一次提交；另有「提交并推送」一步到位）。 */
    ...
    function GitPanel(props) {
      ...
      children.push(h(OutputPanel, { key: 'output', output: s.output, outRef: panel.outRef }))
