...
| 切换 / 刷新 | 「切换」**弹出目录选择小窗口**（与 DSH「添加工作区」同一个选择器：浏览目录、点面包屑回退、双击进入子目录、显示隐藏目录、新建文件夹，也可直接输入绝对路径）；「刷新」重新读取状态 |
...
| 服务缺失时 | — | 退化成「只能手输绝对路径」，并在小窗口里说明原因；面板本体照常工作 |
