...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const S = {
      ...
      body: {
        flex: '1 1 auto', minHeight: 0, padding: '10px',
       ...
       *  未拉伸过时高度固定 72vh（正文区内部滚动）；拉伸过则用记住的固定宽高。 */
      helpwin: {
      ...
      // flex: 1 + minHeight: 0 保证窗口高度受限时正文区可靠收缩并内部滚动
      //（这是 flex column 子项能正确滚动的关键，缺了内容会顶出窗口或滚不动）。
      helpwinBody: {
        flex: '1 1 auto', minHeight: 0, padding: '10px 12px 24px',
    ...
    function GitPanel(props) {
      ...
      if (showHelp === true) {
        const helpChildren = [
          ...
          h('div', { style: S.helpwinBody, key: 'body' },
        ...
        helpWindow = h('div', {
          style: Object.assign({}, S.helpwin,
            ...
            // 从未拉伸过：高度用确定值 72vh（不能用 max-height——flex 子项在有
      // max-height 的容器里不保证收缩，正文滚动会失效，内容会被裁掉）。
      // 拉伸过：用记住的固定宽高。
