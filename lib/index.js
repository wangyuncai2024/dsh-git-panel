...
async function writeNetConfig(input) {
  ...
  try {
    ...
    console.error('[git-panel] 网络加速配置写入失败：' + message(error))
  }
  return merged
}
...
function pullHint(reason) {
  ...
  if (reason === 'remote-branch-missing') {
    return '远端还没有这个分支：先点一次「推送」把它推上去，之后「拉取」才有东西可拉。'
  }
 ...
 *   - 没有远程 / 远端没有这个分支 / 无关历史 / 冲突 → 不做自动动作，只回传 reason，
 *     由 pullHint 告诉用户下一步点哪里。这几类要么缺用户输入，要么会改写历史。
 *
 * @returns { argv, result, reason, retried, note? } —— 与 recoverPush 同形，路由同一处处理。
...
async function recoverPull(op, argv, result, dir, timeoutMs, extraArgs = []) {
  ...
  if (op !== 'pull' || result.code === 0 || reason !== 'no-upstream') {
    return { argv, result, reason, retried: false }
  }
  ...
  if (retry.code !== 0) {
    ...
    return {
      argv: retryArgv,
      result: retry,
      reason: classifyPullFailure(pullFailureText(retry)),
      retried: true,
      remote: chosen,
      branch: branch,
    }
...
async function adoptRemote(body, dir, timeoutMs = 120000) {
  const mode = body !== null && typeof body.mode === 'string' ? body.mode : ''
  ...
  if (mode === 'branch') {
    const wanted = typeof body.name === 'string' && body.name.trim().length > 0
      ? body.name.trim()
...
function createRoutes(getDefaultDir) {
  ...
  const opRoute = {
    ...
    handler: async (request, response) => {
      try {
        ...
        if (op === 'adoptRemote') {
          sendJson(response, 200, { ...(await adoptRemote(body, dir)), state: await readState(dir) })
          return
        }
        if (op === 'abortMerge') {
          sendJson(response, 200, { ...(await abortMerge(dir)), state: await readState(dir) })
          return
        }
        ...
        // 数据型操作的结果：branches 解析成结构化列表，diff 截断到面板能接受的长度。
        // （两者 stdout 原样也会回传，仅供调试；面板渲染用这里的新字段。）
      ...
      try {
        ...
        if (request.method === 'GET') {
          ...
          if (url.searchParams.get('probe') === '1') {
            sendJson(response, 200, { ok: true, results: await probeNetwork(config) })
            return
          }
  ...
  return [stateRoute, opRoute, netRoute, diagRoute, helpRoute]
}
...
export function apply(ctx, config = {}) {
  ...
  /** webServer 是惰性服务：拿到实例后挂上全部路由（state / op / diag / help）。 */
  const mount = (server) => {
...
export {
  ...
  normalizeDir, parseBranchOutput, truncateText, renderHelpHtml, escapeHtml,
