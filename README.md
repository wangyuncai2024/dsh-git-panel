...
| 面板尺寸与折叠 | 左缘可以拖动改宽度（300–520px），宽度与折叠态都记在 `localStorage`，下次打开就是上次的样子；**双击左缘恢复默认宽度**（并把记住的值一起清掉） |
| 底部状态条 | 面板最底部常驻一行：`● main · 3 处改动未提交 · 领先 2` → `origin/main`。左边那颗点的颜色就是**上一次操作的结果**（绿=成功、红=失败、灰=还没操作过），点这一行回到面板顶部（分支管理 + stash + 提交历史全展开时，从底部滑回「改动」要滚很久） |
| 命令结果固定在滚动区之外 | 命令结果、以及「面板需要你选一个结果」的选项都不再挂在正文末尾：正文怎么滚，回音与待决定的事都在视野里（结果区右上角有「清空」可以收起来） |
| diff / 提交详情抬头条 | 展开的 diff 上方先给一行抬头：文件路径（等宽、可省略、tooltip 给全名）+ 「已暂存 / 未暂存」胶囊；提交详情同理（短哈希 + 「提交详情」）。展开的那一行被滚出视野后，抬头就是那块内容的落款 |
| 忙碌指示 | 「同步中…」旁边是一个纯 CSS 的转圈（`prefers-reduced-motion` 下退化成静止小圈）；有未提交改动时，最小化胶囊上的红点会缓慢呼吸 |
...
- **回音与状态常驻，装饰随主题**：命令结果、待选项、底部状态条都在正文滚动区之外 ——
  它们是「刚才那一下成没成」「我现在在哪个分支」，滚走就等于没有。实心按钮的底色与
  文字色都走宿主令牌（`--dsw-alias-button-primary-fill` / `--dsw-alias-label-primary-foreground`）：
  品牌色在浅色主题里接近黑、深色主题里接近白，写死 `#fff` 会变成「白底白字」。
  diff 的新增/删除行用 `color-mix()` 叠一层同色系底纹（不支持时自动退回纯文字着色），
  一屏 diff 可以直接扫。
...
| `GET /git-panel/log` | 无来源校验 | 带 `Origin` 且跨站 → 403（不带 Origin 照常） | 日志是本机工作痕迹，不该被别的站点用脚本拉走 |
| 面板前端 | 渲染异常即静默消失 | 加了渲染错误边界：失败态 + 原因 + 「重试」，原因写进日志 | 与宿主「故障可见」的原则对齐（测试替身下自动降级为直通） |

## 0.13 的界面优化（无契约变更）

这一版**只动客户端半边**（`lib/client.js`）：没有新增 / 改名的接口，没有配置项变化，
装上升级不需要改任何调用方。

| 位置 | 之前 | 现在 |
| --- | --- | --- |
| 命令结果 / 待选项 | 挂在正文末尾，点完按钮可能落在滚动区之外（靠「闪一下」补偿） | 移出正文滚动区固定在底部；结果区加「清空」 |
| 面板底部 | 无 | 常驻状态条：分支 + 改动数 + 领先/落后 + 上游，左边一颗点表示上一次操作的结果；点一下回到顶部 |
| 展开的 diff / 提交详情 | 直接一块代码 | 先给抬头条（路径 + 「已暂存 / 未暂存」；提交详情是短哈希 + 「提交详情」） |
| diff 行 | 只有文字颜色 | 新增/删除再叠一层同色底纹，区块头一条淡蓝带（`color-mix`，不支持时退回纯文字着色） |
| 实心按钮 | 文字写死 `#ffffff` | 底色与文字都走令牌 —— **深色主题下「初始化仓库 / 开始克隆」等实心按钮曾经是白底白字**（看不见） |
| 提交表单 | 输入框与三个按钮挤一行，placeholder 被截成「填写提交信息…（」 | 输入框整行、按钮与勾选项第二行 |
| 主按钮 | 悬停整体调亮度 | 悬停走宿主的 `--dsw-alias-button-primary-hover`（深色主题下品牌色是白的，提亮等于没反应） |
| 标题 / 胶囊 / 状态码 | 次要文字、无徽章 | 区块标题带品牌色短杠并改用 `--dsw-alias-label-caption`；状态码（`M` / `??`）与摘要收成小徽章 |
| 面板出现 / 忙碌 / 折叠 | 无动效 | 面板轻微上浮淡入、转圈（纯 CSS）、脏工作区时胶囊红点呼吸；`prefers-reduced-motion` 下一并关掉 |
| 空态 | 一整块灰字 | 图标 + 说明；「还不是 Git 仓库」等诊断照旧（宿主的 notice 才是原因） |

自查方式（不进仓库）：用 `test/client.test.mjs` 同一套假 React 语义驱动真实 bundle、
把元素树序列化成静态 HTML，用 headless Chrome 分别按浅色 / 深色截图核对版面。
...
`GET /git-panel/log` 是**只读**的日志尾读接口，**带 Origin 的跨站请求一律拒绝**
（地址栏直开、`curl` 不带 Origin，照常可用）。日志本身已经是保守视角：命令参数打码、
凭据不落盘；但它包含操作目录、仓库名与提交信息等**本机工作痕迹**，对局域网开放 web
端口时这些内容同样可见（伪造 Origin 或干脆不带 Origin 的请求仍能读到），
请按下面的结论一并考虑。
...
```bash
npm test        # node --test：205 个用例，秒级完成（假 git / 真 git 的 POSIX 专属用例会自动跳过）
